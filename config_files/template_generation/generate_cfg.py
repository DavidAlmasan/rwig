...
N, M = 12, 3 # Number of underlying states, Number of walkers IN THE SUBSET OF NODES
...
num_trials = 10
epochs = 100
nodes = [1426, 1427, 1428]   # None for all walkers, int for sampling some nodes, list for specific nodes

def generate_cfg(N, M, loss, model_type, dataset, num_trials, epochs, walker_subset, subset_id):
    """--------------------Experiment configuration--------------------"""
    CFG = ConfigDict()
    CFG.NAME = f'[{dataset}][N={N}][M={M}][{loss}][{model_type}][subset_id={subset_id}]'
    CFG.VERBOSE = True
    ...
    CFG.DATA.NODES = walker_subset # None for all walkers, int for sampling some nodes, list for specific nodes
    ### MODEL
